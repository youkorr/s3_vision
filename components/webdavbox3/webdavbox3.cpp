...
// Conversion désormais NEUTRE : on lit/écrit via la couche POSIX/VFS (/sdcard),
// exactement comme le fait déjà fopen() pour GET/PUT.
// On NE force PLUS le lecteur FatFS "0:" : le numéro de volume FatFS attribué à
// la carte SD par esp_vfs_fat_sdmmc_mount() n'est PAS garanti = 0 (il dépend de
// l'ordre de montage / de la plateforme). Sur certaines cartes (ex. ESP32-S3 où
// un autre volume FATFS occupe "0:"), f_readdir("0:") lisait le mauvais volume
// et renvoyait 0 fichier alors que fopen("/sdcard/...") fonctionnait. On renvoie
// donc le chemin VFS tel quel et on n'utilise plus que des appels POSIX.
        ...
        if (*p == '/') {
            *p = 0;
            ...
            if (stat(tmp, &st) != 0) {
                ...
                if (mkdir(fatfs_tmp.c_str(), 0777) != 0 && errno != EEXIST) {
...
void WebDAVBox3::setup() {
  ...
  DIR *dir = opendir(fatfs_root.c_str());
  if (dir == nullptr) {
    ESP_LOGE(TAG, "Impossible d'ouvrir le répertoire racine: %s (errno: %d)", fatfs_root.c_str(), errno);
  } else {
    ...
    while ((entry = readdir(dir)) != nullptr) {
      if (strcmp(entry->d_name, ".") && strcmp(entry->d_name, "..")) {
        ...
        ESP_LOGI(TAG, "  - %s (%s, taille: %lu)",
                ...
                ok ? (unsigned long) st.st_size : 0UL);
      }
    }
    closedir(dir);
  }
...
std::vector<std::string> WebDAVBox3::list_dir(const std::string &path) {
  ...
  std::string fatfs_path = to_fatfs_path(path);
  DIR *dir = opendir(fatfs_path.c_str());
  if (dir != nullptr) {
    struct dirent *entry;
    while ((entry = readdir(dir)) != nullptr) {
      if (strcmp(entry->d_name, ".") && strcmp(entry->d_name, "..")) {
...
esp_err_t WebDAVBox3::handle_webdav_propfind(httpd_req_t *req) {
  ...
  // Vérifier si le chemin existe (POSIX/VFS, comme fopen côté GET/PUT).
  // stat("/sdcard") fonctionne pour la racine, plus besoin du cas spécial "0:".
  std::string fatfs_path = to_fatfs_path(path);
  ...
  file_size = is_directory ? 0 : st.st_size;
  ...
  if (is_directory && (depth_header == "1" || depth_header == "infinity")) {
    ...
    for (const auto &file_name : files) {
      ...
      // stat POSIX/VFS (plus de FatFS brut sur "0:")
      std::string fatfs_file_path = to_fatfs_path(file_path);
      struct stat file_st;
      if (stat(fatfs_file_path.c_str(), &file_st) == 0) {
        ...
        response += generate_prop_xml(href, is_file_dir, file_st.st_mtime, is_file_dir ? 0 : file_st.st_size);
      } else {
...
esp_err_t WebDAVBox3::handle_webdav_get(httpd_req_t *req) {
    ...
    // Vérifier si le fichier existe (POSIX/VFS)
    std::string fatfs_path = to_fatfs_path(path);
    struct stat st;
    if (stat(fatfs_path.c_str(), &st) != 0) {
...
esp_err_t WebDAVBox3::handle_webdav_put(httpd_req_t *req) {
    ...
    std::string fatfs_path = to_fatfs_path(path);
    struct stat st;
    if (stat(fatfs_path.c_str(), &st) == 0 && S_ISDIR(st.st_mode)) {
...
esp_err_t WebDAVBox3::handle_webdav_mkcol(httpd_req_t *req) {
    ...
    // Vérifier si le chemin existe déjà (POSIX/VFS)
    std::string fatfs_path = to_fatfs_path(path);
    struct stat st;
    if (stat(fatfs_path.c_str(), &st) == 0) {
    ...
    // Créer le dossier
    if (mkdir(fatfs_path.c_str(), 0777) != 0 && errno != EEXIST) {
...
esp_err_t WebDAVBox3::handle_webdav_move(httpd_req_t *req) {
  ...
  if (httpd_req_get_hdr_value_str(req, "Destination", dest_uri, sizeof(dest_uri)) == ESP_OK) {
    ...
    if (!parent_dir.empty() && !is_dir(parent_dir)) {
      ...
      if (mkdir(fatfs_parent.c_str(), 0777) != 0 && errno != EEXIST) {
